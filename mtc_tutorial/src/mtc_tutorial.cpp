...
void MTCTaskNode::setupPlanningScene()
{
  ...
  object.primitives[0].dimensions = { 0.035, 0.035, 0.035 }; 

  geometry_msgs::msg::Pose pose;
  if (cube_position_received_)
  {
    ...
    // Default position: in front of robot at reachable height
    pose.position.x = 0.35;
    pose.position.y = 0.0;
    pose.position.z = 0.0;
    pose.orientation.w = 1.0;
    object.pose = pose;
    RCLCPP_WARN(LOGGER, "Cube Position Not Received | Changed to Default Position");
  }
...
mtc::Task MTCTaskNode::createTask()
{
  ...
  // In fact, `task` itself is a SerialContainer by default.
  {
...

    {
      ...
      // Approach from above (world -Z direction)
      geometry_msgs::msg::Vector3Stamped vec;
      vec.header.frame_id = hand_frame;
      vec.vector.z = 1.0;  // Move downward
  ...
  ---- *               Generate Grasp Pose                *
     ***************************************************/
    {
      ...
      // Rotate -90° around X axis to point fingers down
      Eigen::Quaterniond q(Eigen::AngleAxisd(M_PI / 2, Eigen::Vector3d::UnitZ()));
      grasp_frame_transform.linear() = q.matrix();
      grasp_frame_transform.translation().z() = 0.1;  // Palm above cube  
...

    {
      ...
      stage->setGoal({ {"left_finger_prismatic_joint", 0.0017} });
      grasp->insert(std::move(stage));
    }
